...
test_encode(Config) when is_list(Config) ->
    [?assertEqual(Expect, iolist_to_binary(euneus_encoder:encode(Term, Opts)))
     || {Expect, Term, Opts} <- [
        ...
        {<<"null">>, null, #{}},
        {<<"\"foo\"">>, foo, #{}},
        {<<"\"bar\"">>, foo, #{atom => fun(_Term, Encode, _State) -> Encode(<<"bar">>, Encode) end}},
        % Proplist
        ...
        {<<"{\"a\":\"a\",\"b\":\"b\",\"c\":\"c\",\"d\":\"d\",\"e\":\"e\"}">>,
         ...
         #{sort_keys => true}},
        % Datetime
        {<<"\"1970-01-01T00:00:00Z\"">>, {{1970, 01, 01}, {00, 00, 00}}, #{tuple => [datetime]}},
        % Timestamp
        ...
        {<<"[{\"foo\":\"foo\",\"bar\":\"bar\"},{\"bar\":\"bar\",\"baz\":\"baz\"}]">>,
         [#foo{foo = foo, bar = bar},
          #bar{bar = bar, baz = baz}],
         #{tuple => [{record, [{foo, record_info(fields, foo)},
                               {bar, record_info(fields, bar)}]}]}},
        % Pid
        {<<"\"<0.92.0>\"">>,
         list_to_pid("<0.92.0>"),
         #{pid => fun(Pid, Encode, _State) -> Encode(iolist_to_binary(pid_to_list(Pid)), Encode) end}},
        % Port
        {<<"\"#Port<0.1>\"">>,
         list_to_port("#Port<0.1>"),
         #{port => fun(Port, Encode, _State) -> Encode(iolist_to_binary(port_to_list(Port)), Encode) end}},
        % Reference
        {<<"\"#Ref<0.314572725.1088159747.110918>\"">>,
         list_to_ref("#Ref<0.314572725.1088159747.110918>"),
         #{reference => fun(Ref, Encode, _State) -> Encode(iolist_to_binary(ref_to_list(Ref)), Encode) end}}
    ]].
