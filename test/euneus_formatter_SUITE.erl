...
minify(Config) when is_list(Config) ->
    {ok, JSON} = read_file("blockchain.json", Config),
    {ok, Min} = read_file("blockchain.min.json", Config),
    Min = iolist_to_binary(euneus_formatter:minify(JSON)).

prettify(Config) when is_list(Config) ->
    {ok, JSON} = read_file("blockchain.min.json", Config),
    {ok, PP} = read_file("blockchain.json", Config),
...
read_file(Name, Config) ->
    {data_dir, Dir} = proplists:lookup(data_dir, Config),
