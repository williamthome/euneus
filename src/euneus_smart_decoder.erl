...
-compile({ inline, terminate/5 }).
-compile({ inline, key/4 }).
-compile({ inline, key/5 }).
...
-compile({ inline, number_exp_cont/5 }).
...
-compile({ inline, number_frac/5 }).
-compile({ inline, number_frac_cont/5 }).
-compile({ inline, number_minus/4 }).
...
key(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer)
  when H =:= $\s; H =:= $\t; H =:= $\n; H =:= $\r ->
    key(Rest, Input, Pos + 1, Buffer);
key(<<$"/integer,Rest/bitstring>>, Input, Pos, Buffer) ->
    string(Rest, Input, Pos + 1, [?key | Buffer], 0);
key(<<$}/integer,Rest/bitstring>>, Input, Pos, [[] | Buffer]) ->
    continue(Rest, Input, Pos + 1, Buffer, #{});
key(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer) ->
    throw_byte(Rest, Input, Pos, Buffer);
key(<<_/bitstring>>, Input, Pos, Buffer) ->
    throw_eof(Input, Pos, Buffer).

key(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer, Value)
  when H =:= $\s; H =:= $\t; H =:= $\n; H =:= $\r ->
    key(Rest, Input, Pos + 1, Buffer, Value);
key(<<$:/integer,Rest/bitstring>>, Input, Pos, Buffer, Value) ->
    value(Rest, Input, Pos + 1, [?object, Value | Buffer]);
key(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Value) ->
    throw_byte(Rest, Input, Pos, Buffer);
key(<<_/bitstring>>, Input, Pos, Buffer, _Value) ->
    throw_eof(Input, Pos, Buffer).

value(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer)
  when H =:= $\s; H =:= $\t; H =:= $\n; H =:= $\r ->
    value(Rest, Input, Pos + 1, Buffer);
value(<<$\"/integer,Rest/bitstring>>, Input, Pos, Buffer) ->
    string(Rest, Input, Pos + 1, Buffer, 0);
value(<<$-/integer,Rest/bitstring>>, Input, Pos, Buffer) ->
    number_minus(Rest, Input, Pos, Buffer);
value(<<$0/integer,Rest/bitstring>>, Input, Pos, Buffer) ->
    number_zero(Rest, Input, Pos, Buffer, 1);
value(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer)
  when H > $0, H =< $9 ->
    number(Rest, Input, Pos, Buffer, 1);
value(<<"true",Rest/bitstring>>, Input, Pos, Buffer) ->
    continue(Rest, Input, Pos + 4, Buffer, true);
value(<<"false",Rest/bitstring>>, Input, Pos, Buffer) ->
    continue(Rest, Input, Pos + 5, Buffer, false);
value(<<"null",Rest/bitstring>>, Input, Pos, Buffer) ->
...
value(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer) ->
    throw_byte(Rest, Input, Pos, Buffer);
value(<<_/bitstring>>, Input, Pos, Buffer) ->
    throw_eof(Input, Pos, Buffer).

string(<<$"/integer,Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    Value = binary_part(Input, Pos, Len),
    continue(Rest, Input, Pos + Len + 1, Buffer, Value);
string(<<$\\/integer,Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    Part = binary_part(Input, Pos, Len),
    escape(Rest, Input, Pos + Len, Buffer, Part);
string(<<H/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Len)
...
string(<< Y4/integer, Y3/integer, Y2/integer, Y1/integer, $-/integer
        ...
        , Rest0/bitstring >>, Input, Pos, Buffer, 0)
  when ?is_number(Y4), ?is_number(Y3), ?is_number(Y2), ?is_number(Y1)
    ...
    case Rest0 of
        <<$Z/integer, $", Rest/bitstring>> ->
            ...
            Value = {Date, Time},
            continue(Rest, Input, Pos + 21, Buffer, Value);
        << $./integer
        ...
        when ?is_number(MSec3), ?is_number(MSec2), ?is_number(MSec1) ->
            ...
            continue(Rest, Input, Pos + 25, Buffer, Value)
    end;
string(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when H =< ?ONE_BYTE_LAST ->
    string(Rest, Input, Pos, Buffer, Len + 1);
string(<<H/utf8,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when H =< ?TWO_BYTE_LAST ->
    string(Rest, Input, Pos, Buffer, Len + 2);
string(<<H/utf8,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when H =< ?THREE_BYTE_LAST ->
    string(Rest, Input, Pos, Buffer, Len + 3);
string(<<_/utf8,Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    string(Rest, Input, Pos, Buffer, Len + 4);
string(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Len) ->
    throw_byte(Rest, Input, Pos, Buffer);
string(<<_/bitstring>>, Input, Pos, Buffer, _Len) ->
    throw_eof(Input, Pos, Buffer).

string(<<$"/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc, Len) ->
...
string(<<H/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Acc, _Len)
  when H =< ?NON_PRINTABLE_LAST ->
    throw_byte(Rest, Input, Pos, Buffer);
string(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc, Len)
  when H =< ?ONE_BYTE_LAST ->
    string(Rest, Input, Pos, Buffer, Acc, Len + 1);
string(<<H/utf8,Rest/bitstring>>, Input, Pos, Buffer, Acc, Len)
  when H =< ?TWO_BYTE_LAST ->
    string(Rest, Input, Pos, Buffer, Acc, Len + 2);
string(<<H/utf8,Rest/bitstring>>, Input, Pos, Buffer, Acc, Len)
  when H =< ?THREE_BYTE_LAST ->
    string(Rest, Input, Pos, Buffer, Acc, Len + 3);
string(<<_/utf8,Rest/bitstring>>, Input, Pos, Buffer, Acc, Len) ->
    string(Rest, Input, Pos, Buffer, Acc, Len + 4);
string(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Acc, _Len) ->
    throw_byte(Rest, Input, Pos, Buffer);
string(<<_/bitstring>>, Input, Pos, Buffer, _Acc, _Len) ->
    throw_eof(Input, Pos, Buffer).
...
escape(<<$\"/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\"], 0);
escape(<<$//integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $/], 0);
escape(<<$\\/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\\], 0);
escape(<<$b/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\b], 0);
escape(<<$f/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\f], 0);
escape(<<$n/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\n], 0);
escape(<<$r/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\r], 0);
escape(<<$t/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    string(Rest, Input, Pos + 2, Buffer, [Acc, $\t], 0);
escape(<<$u/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    escapeu(Rest, Input, Pos, Buffer, Acc);
escape(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Acc) ->
    throw_byte(Rest, Input, Pos + 1, Buffer);
escape(<<_/bitstring>>, Input, Pos, Buffer, _Acc) ->
    throw_eof(Input, Pos, Buffer).

escapeu(<<12336:16/integer,Int2:16/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc0) ->
    ...
    string(Rest, Input, Pos + 6, Buffer, Acc, 0);
escapeu(<<Int1:16/integer,Int2:16/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc)
  when Int1 > 12336, Int1 < 12344 ->
    ...
    Last = escapeu_last(Int2, Rest, Input, Pos, Buffer),
    escapeu_1(Rest, Input, Pos, Buffer, Acc, Last, X);
escapeu(<<Int1:16/integer,Int2:16/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc)
  ...
  orelse (Int1 > 25903 andalso Int1 < 26215) ->
    ...
    Last = escapeu_last(Int2, Rest, Input, Pos, Buffer),
    escapeu_2(Rest, Input, Pos, Buffer, Acc, Last, X);
escapeu(<<Int1:16/integer,Int2:16/integer,Rest/bitstring>>, Input, Pos, Buffer, Acc) ->
    X = case Int1 of
    ...
    escape_surrogate(Rest, Input, Pos, Buffer, Acc, Hi);
escapeu(<<_Rest/bitstring>>, Input, Pos, Buffer, _Acc) ->
    throw_eof(Input, Pos, Buffer).
...
escape_surrogate(<<92/integer, 117/integer, Int1:16/integer, Int2:16/integer, Rest/bitstring>>,
    ...
    Acc = [Acc0, <<(Hi + Y)/utf8>>],
    string(Rest, Input, Pos + 12, Buffer, Acc, 0);
escape_surrogate(<<_Rest/bitstring>> = Rest, Input, Pos, Buffer, _Acc, _Hi) ->
    throw_byte(Rest, Input, Pos+6, Buffer).
...
number(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when ?is_number(Byte) ->
    number(Rest, Input, Pos, Buffer, Len + 1);
number(<<$./integer,Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    number_frac(Rest, Input, Pos, Buffer, Len + 1);
number(<<E/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when E =:= $e orelse E =:= $E ->
    Prefix = binary_part(Input, Pos, Len),
    number_exp_copy(Rest, Input, Pos + Len + 1, Buffer, Prefix);
number(<<Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    Int = binary_to_integer(binary_part(Input, Pos, Len)),
    continue(Rest, Input, Pos + Len, Buffer, Int).

number_exp(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when ?is_number(Byte) ->
    number_exp_cont(Rest, Input, Pos, Buffer, Len + 1);
number_exp(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when Byte =:= $+ orelse Byte =:= $- ->
    number_exp_sign(Rest, Input, Pos, Buffer, Len + 1);
number_exp(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, Len) ->
    throw_byte(Rest, Input, Pos + Len, Buffer);
number_exp(<<_/bitstring>>, Input, Pos, Buffer, _Len) ->
    throw_eof(Input, Pos, Buffer).

number_exp_cont(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when ?is_number(Byte) ->
    number_exp_cont(Rest, Input, Pos, Buffer, Len + 1);
number_exp_cont(<<Rest/bitstring>>, Input, Pos, Buffer, Len) ->
...
number_exp_cont(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Prefix, Len)
  when ?is_number(Byte) ->
    number_exp_cont(Rest, Input, Pos, Buffer, Prefix, Len + 1);
number_exp_cont(<<Rest/bitstring>>, Input, Pos, Buffer, Prefix, Len) ->
...
number_exp_copy(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Prefix)
  when ?is_number(Byte) ->
    number_exp_cont(Rest, Input, Pos, Buffer, Prefix, 1);
number_exp_copy(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Prefix)
  when Byte =:= $+ orelse Byte =:= $- ->
    number_exp_sign(Rest, Input, Pos, Buffer, Prefix, 1);
number_exp_copy(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Prefix) ->
    throw_byte(Rest, Input, Pos, Buffer);
number_exp_copy(<<_/bitstring>>, Input, Pos, Buffer, _Prefix) ->
    throw_eof(Input, Pos, Buffer).

number_exp_sign(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when ?is_number(Byte) ->
    number_exp_cont(Rest, Input, Pos, Buffer, Len + 1);
number_exp_sign(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, Len) ->
    throw_byte(Rest, Input, Pos + Len, Buffer);
number_exp_sign(<<_/bitstring>>, Input, Pos, Buffer, _Len) ->
    throw_eof(Input, Pos, Buffer).

number_exp_sign(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Prefix, Len)
  when ?is_number(Byte) ->
    number_exp_cont(Rest, Input, Pos, Buffer, Prefix, Len + 1);
number_exp_sign(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Prefix, Len) ->
    throw_byte(Rest, Input, Pos + Len, Buffer);
number_exp_sign(<<_/bitstring>>, Input, Pos, Buffer, _Prefix, _Len) ->
    throw_eof(Input, Pos, Buffer).

number_frac(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when ?is_number(Byte) ->
    number_frac_cont(Rest, Input, Pos, Buffer, Len + 1);
number_frac(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, Len) ->
    throw_byte(Rest, Input, Pos + Len, Buffer);
number_frac(<<_/bitstring>>, Input, Pos, Buffer, _Len) ->
    throw_eof(Input, Pos, Buffer).

number_frac_cont(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when ?is_number(Byte) ->
    number_frac_cont(Rest, Input, Pos, Buffer, Len + 1);
number_frac_cont(<<E/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when E =:= $e orelse E =:= $E ->
    number_exp(Rest, Input, Pos, Buffer, Len + 1);
number_frac_cont(<<Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    Token = binary_part(Input, Pos, Len),
    Float = try_parse_float(Token, Token, Rest, Input, Pos, Buffer),
    continue(Rest, Input, Pos + Len, Buffer, Float).

number_minus(<<$0/integer,Rest/bitstring>>, Input, Pos, Buffer) ->
    number_zero(Rest, Input, Pos, Buffer, 2);
number_minus(<<Byte/integer,Rest/bitstring>>, Input, Pos, Buffer)
  when ?is_number(Byte) ->
    number(Rest, Input, Pos, Buffer, 2);
number_minus(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer) ->
    throw_byte(Rest, Input, Pos + 1, Buffer);
number_minus(<<_/bitstring>>, Input, Pos, Buffer) ->
    throw_eof(Input, Pos, Buffer).

number_zero(<<$./integer,Rest/bitstring>>, Input, Pos, Buffer, Len) ->
    number_frac(Rest, Input, Pos, Buffer, Len + 1);
number_zero(<<E/integer,Rest/bitstring>>, Input, Pos, Buffer, Len)
  when E =:= $e orelse E =:= $E ->
    number_exp_copy(Rest, Input, Pos + Len + 1, Buffer, <<"0">>);
number_zero(<<Rest/bitstring>>, Input, Pos, Buffer, Len) ->
...
object(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer, Value)
  when H =:= $\s; H =:= $\t; H =:= $\n; H =:= $\r ->
    object(Rest, Input, Pos + 1, Buffer, Value);
object(<<$,/integer,Rest/bitstring>>, Input, Pos, [Key, Acc0 | Buffer], Value) ->
    Acc = [{Key, Value} | Acc0],
    key(Rest, Input, Pos + 1, [Acc | Buffer]);
object(<<$}/integer,Rest/bitstring>>, Input, Pos, [Key, Acc0 | Buffer], Value) ->
    Acc = [{Key, Value} | Acc0],
    Map = maps:from_list(Acc),
    continue(Rest, Input, Pos + 1, Buffer, Map);
object(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Value) ->
    throw_byte(Rest, Input, Pos, Buffer);
object(<<_/bitstring>>, Input, Pos, Buffer, _Value) ->
    throw_eof(Input, Pos, Buffer).

array(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer, Value)
  when H =:= $\s; H =:= $\t; H =:= $\n; H =:= $\r ->
    array(Rest, Input, Pos + 1, Buffer, Value);
array(<<$,/integer,Rest/bitstring>>, Input, Pos, [Acc | Buffer], Value) ->
    value(Rest, Input, Pos + 1, [?array, [Value | Acc] | Buffer]);
array(<<$]/integer,Rest/bitstring>>, Input, Pos, [Acc | Buffer], Value) ->
    List = lists:reverse(Acc, [Value]),
    continue(Rest, Input, Pos + 1, Buffer, List);
array(<<_/integer,_/bitstring>> = Rest, Input, Pos, Buffer, _Value) ->
    throw_byte(Rest, Input, Pos, Buffer);
array(<<_/bitstring>>, Input, Pos, Buffer, _Value) ->
    throw_eof(Input, Pos, Buffer).

empty_array(Rest, Input, Pos, [?array, [] | Buffer]) ->
    continue(Rest, Input, Pos, Buffer, []);
...
terminate(<<H/integer,Rest/bitstring>>, Input, Pos, Buffer, Value)
  when H =:= $\s; H =:= $\t; H =:= $\n; H =:= $\r ->
    terminate(Rest, Input, Pos + 1, Buffer, Value);
terminate(<<>>, _Input, _Pos, _Buffer, Value) ->
    Value;
terminate(<<Rest/bitstring>>, Input, Pos, Buffer, _Value) ->
    throw_byte(Rest, Input, Pos, Buffer).
