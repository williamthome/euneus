...
encode(Pid, Opts) when is_pid(Pid) ->
    PidBin = iolist_to_binary(pid_to_list(Pid)),
    {halt, euneus_encoder:encode_binary(PidBin, Opts)};
encode(_Term, _Opts) ->
    next.
...
pid() ->
    list_to_pid(binary_to_list(pid_bin())).
...
encode_plugin_test() ->
    [ ?assertEqual(Expect, euneus:encode(Input, Opts))
      || {Expect, Input, Opts} <- [
        {{ok, [$", pid_bin(), $"]}, pid(), #{plugins => [?MODULE]}}
    ]].

decode_plugin_test() ->
    [ ?assertEqual(Expect, euneus:decode(Input, Opts))
      || {Expect, Input, Opts} <- [
        {{ok, pid()}, <<$", (pid_bin())/binary, $">>, #{plugins => [?MODULE]}}
    ]].
